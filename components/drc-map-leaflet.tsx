...
export function DRCMapLeaflet({ currentProvince = currentProvinceId, className = "" }: DRCMapLeafletProps) {
  ...
  const getProvinceStyle = (feature: any) => {
    ...
    if (isCurrent) {
      ...
      return {
        fillColor: "#10b981", // Vert pour les provinces consultées
        fillOpacity: 0.7,
        color: "#059669",
        weight: 1.5,
        opacity: 0.9,
  ...
  // Calculer le centre approximatif de la RDC (sera ajusté par MapController)
  ...
  return (
    <div className={`relative ${className}`}>
      <div className="relative w-full overflow-hidden rounded-3xl" style={{ position: 'relative' }}>
        ...
        <div className="absolute bottom-6 left-6 z-[1000] rounded-2xl bg-white/80 backdrop-blur-xl p-5 shadow-2xl border border-white/20">
